...
from metrics import per_class_precision
...
def createModel(input_shape, tf_ordering=True, second_training_phase = False):
    ...
    model.add(Convolution2D(64, 3, 3, border_mode='same', init=init, input_shape = input_shape, W_regularizer=l2(l)))
    model.add(LeakyReLU(alpha))

    model.add(Convolution2D(64, 3, 3, border_mode='same', init=init, W_regularizer=l2(l)))
    model.add(LeakyReLU(alpha))
    ...
    model.add(Convolution2D(128, 3, 3, border_mode='same', init=init, W_regularizer=l2(l)))
    model.add(LeakyReLU(alpha))
