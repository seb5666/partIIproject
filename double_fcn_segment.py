...
#    slice = pad(slice, ((height_padding,height_padding),(width_padding, width_padding)), mode='constant')
    ...
    patches2 = []
    diff = int( (patch_size[0] - small_patch_size[0]) / 2)
    for patch in patches1:
        p = patch[diff: diff + small_patch_size[0], diff: diff + small_patch_size[1]]
        patches2.append(p)
    patches2 = np.array(patches2)
    if not(tf_ordering):
        patches1 = np.transpose(patches1, (0,3,2,1))
        patches2 = np.transpose(patches2, (0,3,2,1))
    #not sure if batch size matters
    predictions = []
    i = 0
    while i < patches1.shape[0]:
        if i + batch_size < patches1.shape[0]:
            p1 = patches1[i: i + batch_size]
            p2 = patches2[i: i + batch_size]
        else:
            p1 = patches1[i:]
            p2 = patches2[i:]
        #preds = model.predict_classes(p, batch_size = p.shape[0], verbose=0)
        preds_prob = model.predict([p2, p1], batch_size = p.shape[0], verbose=0)
